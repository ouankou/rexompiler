...
bool ClangToSageTranslator::VisitStmt(clang::Stmt * stmt, SgNode ** node)
   {
#if DEBUG_VISIT_STMT
     std::cerr << "ClangToSageTranslator::VisitStmt" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitAsmStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitGCCAsmStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitMSAsmStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitBreakStmt" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCapturedStmt" << "\n";
#endif
    bool res = true;
...
bool ClangToSageTranslator::VisitCompoundStmt(clang::CompoundStmt * compound_stmt, SgNode ** node) {
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCompoundStmt" << "\n";
#endif
    ...
    for (it = compound_stmt->body_begin(); it != compound_stmt->body_end(); it++) {
        ...
        if (tmp_node != NULL)
          ...
          std::cerr << "In VisitCompoundStmt : child is NULL" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitContinueStmt" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCoreturnStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCoroutineBodyStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXCatchStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXForRangeStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXTryStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitDeclStmt" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
        std::cerr << "In VisitDeclStmt(): *node = " << *node << " = " << (*node)->class_name().c_str() << "\n";
#endif
    }
    else {
...
#if DEBUG_VISIT_STMT
    std::cerr << "In VisitDeclStmt(): identify where the parent is not set: *node = " << *node << " = " << (*node)->class_name().c_str() << "\n";
    std::cerr << " --- *node parent = " << (*node)->get_parent() << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitDoStmt" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitForStmt" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "In VisitForStmt(): Setting the parent of the sg_for_stmt" << "\n";
#endif
  ...
  // Initialization
...

    {
        ...
        if (tmp_init != NULL && init_stmt == NULL && init_expr == NULL) {
            std::cerr << "Runtime error: tmp_init != NULL && init_stmt == NULL && init_expr == NULL (" << tmp_init->class_name() << ")" << "\n";
            res = false;
        }
...
#if DEBUG_VISIT_STMT
        std::cerr << "In VisitForStmt(): for_init_stmt = " << for_init_stmt << "\"n";
#endif
  ...
  // Increment
...

    {
        ...
        if (tmp_inc != NULL && inc == NULL) {
            std::cerr << "Runtime error: tmp_inc != NULL && inc == NULL" << "\n";
            res = false;
        }
        if (inc == NULL) {
...
bool ClangToSageTranslator::VisitGotoStmt(clang::GotoStmt * goto_stmt, SgNode ** node) {
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitGotoStmt" << "\n";
#endif
    ...
    if (sym == NULL) {
        ...
        if (label_stmt == NULL) {
            std::cerr << "Runtime error: Cannot find the symbol for the label: \"" << goto_stmt->getLabel()->getStmt()->getName() << "\"." << "\n";
            std::cerr << "Runtime Error: Cannot find the label: \"" << goto_stmt->getLabel()->getStmt()->getName() << "\"." << "\n";
            res = false;
        }
        else {
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitIfStmt" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitIndirectGotoStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitMSDependentExistsStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitNullStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPAtomicDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPBarrierDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPCancelDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPCriticalDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPFlushDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPLoopDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPDistributeParallelForSimdDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPForDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPForSimdDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPSimdDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPTargetTeamsDistributeSimdDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPTaskLoopDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPMasterDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPOrderedDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPParallelDirective" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitReturnStmt" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitSEHExceptStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitSEHFinallyStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitSEHLeaveStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitSEHTryStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitSwitchCase" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCaseStmt" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitDefaultStmt" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitSwitchStmt" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitValueStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitAttributedStmt" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitExpr" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitConditionalOperator" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitAddrLabelExpr" << "\n";
#endif
     bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitArrayInitIndexExpr" << "\n";
#endif
     bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitArrayInitLoopExpr" << "\n";
#endif
     bool res = true;
...
bool ClangToSageTranslator::VisitArraySubscriptExpr(clang::ArraySubscriptExpr * array_subscript_expr, SgNode ** node) {
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitArraySubscriptExpr" << "\n";
#endif
    ...
    if (tmp_idx != NULL && idx == NULL) {
        std::cerr << "Runtime error: tmp_idx != NULL && idx == NULL" << "\n";
        res = false;
    }
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitArrayTypeTraitExpr" << "\n";
#endif
     bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitAsTypeExpr" << "\n";
#endif
     bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitAtomicExpr" << "\n";
#endif
     bool res = true;
...
bool ClangToSageTranslator::VisitBinaryOperator(clang::BinaryOperator * binary_operator, SgNode ** node) {
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitBinaryOperator" << "\n";
#endif
    ...
    if (tmp_lhs != NULL && lhs == NULL) {
        std::cerr << "Runtime error: tmp_lhs != NULL && lhs == NULL" << "\n";
        res = false;
    }
    ...
    if (tmp_rhs != NULL && rhs == NULL) {
        std::cerr << "Runtime error: tmp_rhs != NULL && rhs == NULL" << "\n";
        res = false;
    }
    ...
    switch (binary_operator->getOpcode()) {
        ...
        default:
            std::cerr << "Unknown opcode for binary operator: " << binary_operator->getOpcodeStr().str() << "\n";
            res = false;
    }
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitBlockExpr" << "\n";
#endif
     bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCallExpr" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCUDAKernelCallExpr" << "\n";
#endif
     bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXMemberCallExpr" << "\n";
#endif
     bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXOperatorCallExpr" << "\n";
#endif
     bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitUserDefinedLiteral" << "\n";
#endif
     bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCastExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitExplicitCastExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitBuiltinBitCastExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCStyleCastExpr" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXFunctionalCastExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXNamedCastExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXConstCastExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXDynamicCastExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXStaticCastExpr" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitImplicitCastExpr" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCharacterLiteral" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitChooseExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCompoundLiteralExpr" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitConvertVectorExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCoroutineSuspendExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCoawaitExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCoyieldExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXBindTemporaryExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXBoolLiteralExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXConstructExpr" << "\n";
    // isElidable seems to be related to copy elision: https://en.cppreference.com/w/cpp/language/copy_elision
    std::cerr << "isElidable " << cxx_construct_expr->isElidable() << "\n";
    std::cerr << "hadMultipleCandidates " << cxx_construct_expr->hadMultipleCandidates() << "\n";
    std::cerr << "isListInitialization " << cxx_construct_expr->isListInitialization() << "\n";
    std::cerr << "isStdInitListInitialization " << cxx_construct_expr->isStdInitListInitialization() << "\n";
    std::cerr << "requiresZeroInitialization " << cxx_construct_expr->requiresZeroInitialization() << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "clang::CXXConstructExpr: is default constructor: " << cxx_constructor_decl->isDefaultConstructor() << "\n";
    std::cerr << "clang::CXXConstructExpr: is from UnNamed class: " << enclosingClassDecl->get_isUnNamed() << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXDefaultArgExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXDefaultInitExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXDeleteExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXFoldExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXNewExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXNoexceptExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXNullPtrLiteralExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXThisExpr" << "\n";
    std::cerr << "isImplicit: " << cxx_this_expr->isImplicit() << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXThrowExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXTypeidExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitCXXUuidofExpr" << "\n";
#endif
    bool res = true;
...
bool ClangToSageTranslator::VisitDeclRefExpr(clang::DeclRefExpr * decl_ref_expr, SgNode ** node) {
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitDeclRefExpr" << "\n";
    std::cerr << "hasQualifier " << decl_ref_expr->hasQualifier() << "\n";
#endif
    ...
    if(decl_ref_expr->hasQualifier())
    {
      ...
      else
      {
        ...
        {
          std::cerr << "No valid qualifier found!\n";
          ROSE_ASSERT(0);
        }
      }
    }
    ...
    // FIXME: I feel like it could work now, we will see ....
...
#if DEBUG_VISIT_STMT
          std::cerr << "temp_decl = " << tmp_decl << " = " << tmp_decl->class_name().c_str() << "\n";
#endif
          SgInitializedName* initializedName = isSgInitializedName(tmp_decl);
#if DEBUG_VISIT_STMT
          if (initializedName != NULL)
             {
               std::cerr << "Found SgInitializedName: initializedName->get_name() = " << initializedName->get_name().str() << "\n";
             }
#endif
     ...
     if (sym != NULL) 
       ...
       // Not else if it was NULL we have try to traverse it....
                 ...
                 else
                  {
                    if (enum_sym != NULL)
                      ...
                      else
                       {
                         if (sym != NULL)
                            {
                              std::cerr << "Runtime error: Unknown type of symbol for a declaration reference." << "\n";
        ...
        {
          std::cerr << "Runtime error: Cannot find the symbol for a declaration reference (even after trying to buil th declaration)" << "\n";
          ROSE_ABORT();
        }
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitDependentCoawaitExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitDesignatedInitExpr" << "\n";
#endif
    ...
    for (auto it=0; it < designatorSize; it++) {
        ...
        if (D->isFieldDesignator()) {
            ...
            if(clang::ConstantExpr::classof(designated_init_expr->getArrayIndex(*D)))
            {
               ...
               clang::IntegerLiteral* integerLiteral = (clang::IntegerLiteral*) fullExpr->getSubExpr();
               tmp_expr = SageBuilder::buildUnsignedLongVal((unsigned long) integerLiteral->getValue().getSExtValue());
std::cerr << "idx:" << integerLiteral->getValue().getSExtValue() << "\n";
            }
            else
            {
               tmp_expr = Traverse(designated_init_expr->getArrayIndex(*D));
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitExpressionTraitExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitExtVectorElementExpr" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitFixedPointLiteral" << "\n";
#endif
    bool res = true;
...
bool ClangToSageTranslator::VisitFloatingLiteral(clang::FloatingLiteral * floating_literal, SgNode ** node) {
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitFloatingLiteral" << "\n";
#endif
    ...
    llvmFloat.print(os);
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitFloatingLiteral precision = " << precision  << "\n";
    std::cerr << "ClangToSageTranslator::VisitFloatingLiteral value in string = " << os.str()  << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitFullExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr <<  "In VisitFullExpr(): built: expr = " << expr << " = " << expr->class_name().c_str() << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitConstantExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitExprWithCleanups" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitFunctionParmPackExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitGenericSelectionExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitGNUNullExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitImaginaryLiteral" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitImplicitValueInitExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitInitListExpr" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitIntegerLiteral" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitLambdaExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitMemberExpr" << "\n";
...
//        if(classDecl->get_isUnNamed())
        {
          ...
          std::cerr << "build varName:" << varName << "\n";
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitMSPropertyRefExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitNoInitExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOffsetOfExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOMPArraySectionExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOpaqueValueExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitOverloadExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitUnresolvedLookupExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitUnresolvedMemberExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitPackExpansionExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitParenExpr" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitParenListExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitPredefinedExpr" << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitPseudoObjectExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitShuffleVectorExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitSizeOfPackExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitSourceLocExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitStmtExpr" << "\n";
#endif
...
bool ClangToSageTranslator::VisitStringLiteral(clang::StringLiteral * string_literal, SgNode ** node) {
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitStringLiteral" << "\n";
#endif
    bool res = true;
    ...
    std::string newstr;
#if DEBUG_VISIT_STMT
    std::cerr << "In ClangToSageTranslator string_literal length: " << string_literal->getLength() << " byteLength:" << string_literal->getByteLength() << "\n";
#endif
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitTypeTraitExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitTypoExpr" << "\n";
#endif
    bool res = true;
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitUnaryOperator" << "\n";
#endif
    ...
    switch (unary_operator->getOpcode()) {
        ...
        case clang::UO_Extension:
            *node = subexpr;
            break;
        default:
            std::cerr << "Runtime error: Unknown unary operator." << "\n";
            res = false;
    }
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitVAArgExpr" << "\n";
#endif
...
bool ClangToSageTranslator::VisitLabelStmt(clang::LabelStmt * label_stmt, SgNode ** node) {
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitLabelStmt" << "\n";
#endif
    ...
    *node = SageBuilder::buildLabelStatement_nfi(name, NULL, SageBuilder::topScopeStack());
    ...
    if (label_scope == NULL) {
         std::cerr << "Runtime error: Cannot find a surrounding function definition for the label statement: \"" << name << "\"." << "\n";
         res = false;
    }
    else {
...
#if DEBUG_VISIT_STMT
    std::cerr << "ClangToSageTranslator::VisitWhileStmt" << "\n";
#endif
